...
export class NotificationService {
  ...
  static async getScheduledNotifications() {
    try {
      ...
      triggerNotifications.forEach((notification, index) => {
        ...
        console.log(`${index + 1}. ID: ${notification.notification.id}, Time: ${date}, Title: ${notification.notification.title}`);
      });
      
      console.log(`Displayed notifications: ${displayedNotifications.length}`);
      displayedNotifications.forEach((notification, index) => {
        console.log(`${index + 1}. ID: ${notification.id}, Title: ${notification.title}`);
      });
  ...
  static async scheduleAllEnabledNotifications(schedules: Schedule[]) {
    try {
      ...
      enabledSchedules.forEach(s => this.log(`  - ${s.name}: enabled=${s.isEnabled}`));
      ...
      for (const [scheduleIndex, schedule] of enabledSchedules.entries()) {
        this.log(`Processing schedule ${scheduleIndex + 1}/${enabledSchedules.length}: ${schedule.name}`);
        await this.scheduleNotificationsForSchedule(schedule, scheduleIndex * 1000);
      }
  ...
  private static async scheduleNotificationsForSchedule(
    ...
    try {
      this.log(`  Schedule: ${schedule.name}, hour: ${schedule.hour}, minute: ${schedule.minute}, daysOfWeek: [${schedule.daysOfWeek?.join(',')}], motivation: ${schedule.motivationPercentage}%`);
      ...
      this.log(`  Setting up schedule: ${schedule.name} for ${enabledDays} days`);
  ...
  private static async scheduleWeeklyNotifications(
    ...
    for (let weekOffset = 0; weekOffset < 2; weekOffset++) {
      for (let dayOfWeek = 0; dayOfWeek < schedule.daysOfWeek.length; dayOfWeek++) {
        ...
        if (scheduledTime > now) {
          ...
          await this.scheduleNotification(notificationId, scheduledTime, schedule);
        }
      }
    }
  }
  ...
  private static async scheduleNotification(
    ...
    if (delay > 0) {
      this.log(`    Notification ${id} scheduled for ${scheduledTime.toLocaleString()} (in ${Math.round(delay / 1000)}s)`);
  ...
  private static async scheduleBackgroundNotification(
    ...
    try {
      ...
      this.log(`    Background trigger notification ${id} scheduled: ${messageContent}`);
    } catch (error) {
